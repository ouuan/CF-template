...
...Args>void read(char*x,Args&...args){read(x);read(args...);}void wts(const char*x){write(x);sp;}void
wtb(const char*x){write(x);br;}template<typename T>void wts(const T&x){write(x);sp;}template<typename
T>void wtb(const T&x){write(x);br;}template<typename...Args>void wts(const char*x,Args...args){wts(
x);wts(args...);}template<typename...Args>void wtb(const char*x,Args...args){wts(x);wtb(args...);}template
<typename T,typename...Args>void wts(const T&x,Args...args){wts(x);wts(args...);}template<typename T
,typename...Args>void wtb(const T&x,Args...args){wts(x);wtb(args...);}template<class T> inline bool
up(T&x,const T&y){return x<y?x=y,1:0;}template<class T> inline bool dn(T&x,const T&y){return y<x?x=y,1:0;}
